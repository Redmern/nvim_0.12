...
- Claude terminal: `<C-j>`/`<C-k>` stay window/pane **nav** (smart-splits' global maps); scrolling is buffer-local **`<A-k>`/`<A-j>`** (`plugins/claudecode.lua`), shadowing the global Alt+j/k split resize there. They replay mouse-wheel events over the Claude window (`nvim_input_mouse`) — the wheel scrolls under both renderers (fullscreen Claude via mouse tracking, classic via nvim scrollback); PageUp/PageDown did nothing. `<A-n>` (t-mode, buffer-local) = `<C-\><C-n>` to select/copy output — only the visible screen under fullscreen Claude. `<C-h>` still leaves the pane. Shift+Enter: WezTerm (`~/.wezterm.lua`) sends it as CSI-u `<S-CR>` when the pane runs nvim (elsewhere as `<C-j>`, Claude's newline); the Claude buffer maps `<S-CR>` to a raw LF so it can't collide with `<C-j>` nav.
